...
def constraints(offsets, parents, translations, **kwargs):
    """
    ...
    points_offsets = offsets
    points = translations
    
    if masses is None:
        masses = 1.0 / (0.1 + np.absolute(points_offsets[:,1]))
        masses = masses[np.newaxis].repeat(len(parents), axis=0)
...
def graph(offsets, parents):
    ...
    graph = np.zeros(offsets.shape[0], offsets.shape[0])
    lengths = np.sum(offsets**2.0, axis=1)**0.5 + 0.001
...
def distances(offsets, parents):
    ...
    distances = np.zeros((offsets.shape[0], offsets.shape[0]))
    generated = distances.copy().astype(bool)
    
    joint_lengths = np.sum(offsets**2.0, axis=1)**0.5
    joint_children = children_list(parents)
    joint_parents  = parents_list(parents)
