...
class Quaternions:
    ...
    def average(self):
        ...
        if len(self.shape) == 1:
            system = (self.qs[:,:,np.newaxis] @ self.qs[:,np.newaxis,:]).sum(axis=0)
    ...
    @classmethod
    def from_transforms2(cls, ts, rtol=1e-05, atol=1e-08):
        """
        This method is fucking slow

        ts.shape = (4, 4)   # TODO: implement for (..., 4, 4) case
        """
        try:
            shape = ts.shape
        ...
        forward = normalized(ts[:3, 2])
        up = ts[:3, 1]
        left = normalized(np.cross(up, forward))
