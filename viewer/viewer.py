...
class SimpleViewer(ShowBase):
    def __init__(self, fStartDirect=True, windowType=None):
        ...
        # def cal_rot(up, offset):
        #     rot_axis = np.cross(offset, up[np.newaxis, :])
        #     print(rot_axis.shape)
        #     rot_axis = rot_axis / np.linalg.norm(rot_axis)
        #     angle = np.arccos(np.dot(offset, up) / (np.linalg.norm(up) * np.linalg.norm(offset)))
        #     return R.from_rotvec(rot_axis * angle).as_quat()
        ...
        # rot = R.from_euler('XYZ', [90, 0, 0], degrees=True).as_quat()
        up = np.array([0, 1, 0])
        offset = np.array([1, 0, 0])
        rot_axis = np.cross(up, offset)
        rot_axis = rot_axis / np.linalg.norm(rot_axis)
        angle = np.arccos(np.dot(offset, up) / (np.linalg.norm(up) * np.linalg.norm(offset)))
        rot = R.from_rotvec(rot_axis * angle).as_quat()

        # self.add_text("Press 'v' to show buffer viewer")
        # self.bufferViewer = self.win.makeBufferViewer()

        # self.create_link(0, [0, 1, 0], [0.1, 1, 0.1], rot)
        pass

    def add_text(self, text, pos=(0, 0), scale=0.05, color=(1, 1, 1, 1)):
        self.text = OnscreenText(text=text, pos=pos, scale=scale, fg=color)
        self.text.setBin('fixed', 0)
        self.text.setDepthTest(True)
        self.text.setDepthWrite(True)
        self.text.reparentTo(self.render2d)
    ...
    def load_ground(self):
        self.ground = self.loader.loadModel("data/materials/GroundScene.egg")
    ...
    def create_joint(self, link_id, position, end_effector=False):
        # create a joint
        box = self.loader.loadModel("data/materials/GroundScene.egg")
    ...
    def load_character(self, info: Dict[str, List | np.ndarray]):
        joint_pos = info['joint_pos']
        joint_offset = info['joint_offset']
        ...
        for name in joint_name:
            if 'end' in name:
                ...
                name_idx_map[name] = len(name_idx_map)
        # print(body_pos.shape)
        scale = [ [thickness, np.linalg.norm(joint_offset[i + 1]) / 2, thickness] for i in range(len(body_pos))]
        
        # joint_pos = np.concatenate([body_pos[0:1], joint_pos], axis=0)
        offset = 0
        joint_idx_offset = [0] * len(joint_name)
        for i in range(len(joint_pos)):
            joint_idx_offset[i] = offset
            if 'end' in joint_name[i]:
                offset += 1
            else:
                # print(f"joint {i}: {joint_name[i]} {joint_pos[i]}")
                joint.append(self.create_joint(i, joint_pos[i], 'end' in joint_name[i]))
            if i < len(body_pos):
                if 'end' in joint_name[i + 1]:
                    continue
                body.append(self.create_link(i, body_pos[i], scale[i], rot=body_rot[i + 1] if body_rot is not None else None))
                body[-1].wrtReparentTo(joint[parent[i + 1] - joint_idx_offset[parent[i + 1]]])
        # print(len(joint), len(body))
        # exit(0)
    ...
    def create_marker(self, pos, color):
        ...
        marker = self.loader.loadModel("data/materials/GroundScene.egg")
...
class ShowBVHUpdate():
    ...
    def update(self, task):
        ...
        for i in range(len(self.joint_name)):
            self.viewer.set_joint_position_orientation(
                self.joint_name[i],
                self.translation[self.cur_frame // speed_inv, i, :],
                self.orientation[self.cur_frame // speed_inv, i, :])
        self.cur_frame = (self.cur_frame + 1) % (self.translation.shape[0] *
                                                 speed_inv)
        return task.cont